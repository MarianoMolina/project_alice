...
const CodeGenerationLLMTask: React.FC<TaskFormProps<CodeGenerationLLMTaskForm>> = ({ form, setForm, viewOnly }) => {
  const handleBaseFormChange = (newBaseForm: PromptAgentTaskForm) => {
    setForm({ ...form, ...newBaseForm });
  };
  return (
    <Box>
      <PromptAgentTask form={form} setForm={handleBaseFormChange} viewOnly={viewOnly} />
    </Box>
  );
};
