...
class ChatExecutionFunctionality(BaseModel):
    ...
    async def take_turn(self, messages: List[MessageDict]) -> Tuple[List[MessageDict], bool]:
        ...
        while recursion_depth < self.max_recursion_depth:
            chat_response = await self.llm_agent.a_generate_reply(messages + new_messages)
            ...
            if isinstance(chat_response, dict) and chat_response.get("function_call"):
                ...
                code_messages = await self.handle_potential_code_execution(chat_response)
                if code_messages:
    ...
    async def handle_function_call(self, response: dict) -> List[MessageDict]:
        ...
        is_success, result = await self.execution_agent.execute_function(function_call)
