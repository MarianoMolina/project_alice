...
from workflow_logic.core.tasks.web_scrapping_tasks.web_scrape_utils import fetch_webpage, preprocess_html, sample_html, fallback_parsing_strategy, apply_parsing_strategy
from workflow_logic.core.data_structures import ApiType, References, TaskResponse, References, MessageDict, FunctionParameters, ParameterDefinition, ContentType, URLReference
...
class WebScrapeBeautifulSoupTask(BasicAgentTask):
    ...
    async def retrieve_and_parse_webpage(self, url: str, api_manager: APIManager) -> URLReference:
        ...
        try:
            ...
            selectors, completion_metadata = await self.generate_parsing_instructions(html_samples, api_manager)
            if selectors:
                LOGGER.info(f"Selectors generated by the agent: {selectors}")
                content = apply_parsing_strategy(cleaned_html, selectors)
                if content:
                    return URLReference(url=url, content=content, metadata={"selectors": selectors, "completion_metadata": completion_metadata})
                else:
            ...
            try:
                ...
                return URLReference(url=url, content=content, metadata={"selectors": ["p"]})
    ...
    """
            ...
            try:
                ...
                completion_metadata[f"sample_{idx}"] = new_messages[-1].completion_metadata if new_messages else None
                LOGGER.debug(f"LLM response: {instructions}")
                # Parse the JSON output using Pydantic
                selector_model = SelectorModel.model_validate(instructions)
        ...
        return unique_selectors if unique_selectors else None, completion_metadata if completion_metadata else None
