...
class OpenAITextToSpeechEngine(APIEngine):
    ...
    async def generate_api_response(self, api_data: LLMConfig, input: str, voice: str = "alloy", speed: float = 1.0) -> MessageDict:
        """
        ...
        try:
            ...
            file_reference = FileContentReference(
                ...
                transcript=MessageDict(role='tool', content=f"Speech generated by model {model}. /nInput: '{input}' /nVoice: {voice}", type=ContentType.TEXT, generated_by='tool', creation_metadata=creation_metadata)
            )
