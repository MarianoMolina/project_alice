...
from workflow_logic.core.prompt.prompt import Prompt

class PromptLibrary(BaseModel):
    template_map: Dict[str, Prompt] = Field(..., description="A dictionary mapping template names to prompts.")
    ...
    def get_prompt_by_name(self, template_name: str) -> Prompt:
    ...
    def update_prompt(self, name: str, prompt: Prompt) -> None:
        if name not in self.template_names:
            raise ValueError(f"No prompt found with the name '{name}'.")
        self.template_map[name] = prompt
